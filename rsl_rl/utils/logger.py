...
class Logger:
    ...
    def _format_step_metric(self, value: torch.Tensor) -> torch.Tensor:
        """Normalize a per-step metric to shape ``[num_envs, 1]`` for accumulation."""
    ...
    def _get_metric_sum(self, name: str) -> torch.Tensor:
        """Return the rolling per-env accumulator for a named metric, creating it if needed."""
    ...
    def process_env_step(
        ...
        # Update per-step metrics and episode length. Plugins can contribute
        # additional values through ``extras["step_metrics"]``.
        ...
        # Update intrinsic reward bookkeeping
        ...
        if self.writer is not None:
            ...
            if len(self.rewbuffer) > 0:
                for name, metric_buffer in self.metric_buffers.items():
                    if len(metric_buffer) > 0:
                        log_string += (
                            f"""{self._console_metric_label(name):>{pad}} {statistics.mean(metric_buffer):.2f}\n"""
                        )
