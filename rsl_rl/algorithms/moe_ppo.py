...
class MoEPPO:
    ...
    actor: StochasticWrapper | BackboneMoE
    """The actor model."""
    ...
    def load(self, loaded_dict: dict, load_cfg: dict | None, strict: bool) -> bool:
        ...
        if load_cfg.get("actor"):
            _load_state_dict_compat(self.actor, loaded_dict["actor_state_dict"])
        if load_cfg.get("critic"):
            _load_state_dict_compat(self.critic, loaded_dict["critic_state_dict"])
    ...
    #region training router only
    def freeze_experts(self):
        # the optimizer will only update the router's parameters
        actor_backbone = self.actor.backbone if hasattr(self.actor, "backbone") else self.actor
        if not isinstance(actor_backbone, BackboneMoE):
            raise TypeError(
                f"freeze_experts() requires actor backbone to be BackboneMoE, got {type(actor_backbone)!r}."
            )
        actor_backbone.freeze_experts()

    def load_experts(self, experts):
        # load experts from given checkpoint
        # TODO make the load_experts function more flexible
        actor_backbone = self.actor.backbone if hasattr(self.actor, "backbone") else self.actor
        if not isinstance(actor_backbone, BackboneMoE):
            raise TypeError(
                f"load_experts() requires actor backbone to be BackboneMoE, got {type(actor_backbone)!r}."
            )
        actor_backbone.load_experts(experts)
    #endregion
